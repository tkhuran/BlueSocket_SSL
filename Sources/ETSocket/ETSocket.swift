...
public class ETSocket: ETReader, ETWriter {
  ...
  /// SSL module
  // public private(set) var ssl: SwiftSSL = SwiftSSL()
  private let ssl = SwiftSSL()
  
  /// 
  /// SSL context
  ///
  public private(set) var sslContext: SSLContext?
    
  ///
  /// SSL connection
  ///
  public private(set) var sslConnection:SSLConnection?
  ...
  public class func defaultSSLConfigured(credentials: SSLCredentials) throws -> ETSocket {
  ...
  /// - Returns: New ETSocket instance
  ///
  private init(family: SocketProtocolFamily, type: SocketType, proto: SocketProtocol, credentials: SSLCredentials? = nil) throws {
    ...
    if (isSSL) {
      ...
      let sslCredentials = SSLCredentials(certificateFile: "certificate.pem", keyFile: "key.pem")
      sslContext = ssl.createContext(sslCredentials)
      // try createSSLContext(usingCertificate: nil, andKey: nil)

      sslConnection = sslContext?.createConnection()
      sslConnection?.fileDescriptor = socketfd2

      do {
	try sslConnection?.accept()
      } catch {
